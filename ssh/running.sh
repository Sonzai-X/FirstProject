...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/Sonzai-X/X/main/Note > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/Sonzai-X/X/main/Note | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/Sonzai-X/X/main/Note | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
Exp=$(curl -sS https://raw.githubusercontent.com/Sonzai-X/X/main/Note | grep $MYIP | awk '{print $3}')
fi
clear
MYIP=$(curl -sS ipv4.icanhazip.com)
#########################
